...
(function () {
    ...
    var BT_PREF_INJECT_JS = [
        ...
        "        h += '<input class=\\'bt-setting-input\\' data-action=\\'setting\\' data-key=\\'' + s.key + '\\' data-type=\\'number\\' type=\\'number\\'' + (s.min !== undefined ? ' min=\\'' + s.min + '\\'' : '') + (s.max !== undefined ? ' max=\\'' + s.max + '\\'' : '') + ' value=\\'' + (val === undefined || val === null ? '' : val) + '\\'>';",
        ...
        "    if (!sid3) return;",
        "    var val = inp.value;",
        "    if (inp.getAttribute('data-type') === 'number') {",
        "      val = parseFloat(val) || 0;",
        "      // 手动输入可绕过 min/max 原生提示 → 提交时夹取到有效范围",
        "      var mn = parseFloat(inp.getAttribute('min'));",
        "      var mx = parseFloat(inp.getAttribute('max'));",
        "      if (!isNaN(mn) && val < mn) val = mn;",
        "      if (!isNaN(mx) && val > mx) val = mx;",
        "      if (String(inp.value) !== String(val)) inp.value = val;   // 回写修正显示",
        "    }",
